...
const getAllPromocionesByProductorId = async (req, res) => {
  try {
    ...
    // Consulta que trae SOLO descuentos (promociones) del agricultor
    // Usa INNER JOIN para garantizar que exista un descuento
    let sql = `
      SELECT 
      ...
      -- Enlazar con Descuentos (INNER JOIN para SOLO traer descuentos que existen)
      INNER JOIN 
